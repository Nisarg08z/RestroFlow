...
import {
    ArrowLeft, DollarSign, Receipt, Settings, Users, Clock
} from "lucide-react";
        ...
        <>
                ...
                <div className="w-full px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center justify-between md:justify-end gap-6 w-full md:w-auto">
                            {isOpen && currentView === "cards" && onOpenLocationStatus && (
                                <button
                                    onClick={onOpenLocationStatus}
                                    className="px-4 py-2.5 rounded-xl border border-border/70 bg-muted/40 hover:bg-muted text-sm font-semibold text-foreground transition-colors shadow-sm"
                                >
                                    Location Status
                                </button>
                            )}
                            ...
                            {isOpen && currentView === 'manager' && (
                                <div className="hidden md:flex items-center p-1 bg-muted/30 rounded-2xl border border-border/50 max-w-[90vw] md:max-w-full overflow-x-auto scrollbar-hide shrink-0">
                                    {tabs.map((tab) => {
                                        const Icon = tab.icon;
            ...
            {isOpen && currentView === 'manager' && (
                <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] bg-card/90 backdrop-blur-xl border-t border-border shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.1)]">
                    <div className="flex items-center overflow-x-auto scrollbar-hide px-2 py-2 gap-1 pb-safe snap-x">
                        {tabs.map((tab) => {
                            const Icon = tab.icon;
                            const isActive = activeTab === tab.id;
                            ...
                            return (
                                <button
                                    key={tab.id}
                                    onClick={() => onTabChange(tab.id)}
                                    className={`relative snap-start flex flex-col items-center justify-center gap-1.5 min-w-[72px] flex-1 py-2 rounded-xl transition-all duration-300 ${isActive
                                        ? "text-primary"
                                        : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                                        }`}
                                >
                                    ...
                                    <span className="text-[10px] font-bold tracking-wide relative z-10 text-center truncate w-full px-1">
                                        {tab.label}
                                    </span>
                                </button>
                            );
